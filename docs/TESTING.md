...
| macOS helper runtime peer credentials | macOS-only tests call `getpeereid` on both endpoints of a real connected `UnixStream::pair`, compare returned effective UID/GID with process kernel credentials, cover stable invalid-descriptor failure and prove configured UID mismatch stops admission before right/session callbacks | persistent filesystem listener, cross-process helper/client, different-UID process, same-UID attacker, socket owner/mode lifecycle, Security framework authorization, root runtime and network mutation |
