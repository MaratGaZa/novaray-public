...
| Network transaction contract | pure `NetworkSnapshot` / `AppliedNetworkState` validation covers route/DNS/firewall snapshots, typed operation descriptors, transaction phases, duplicate operation/route rejection, bounded identifiers, missing rollback metadata rejection, explicit `apply_order`, reverse-order rollback step generation, failed/rolled-back semantics, unknown-field rejection, redacted `Debug` output, typed recovery journal persistence with private directory/temp-write/fsync/rename/fail-closed per-file quarantine/orphan temp cleanup/file-safe transaction IDs/explicit clear/redacted journal diagnostics, full-tunnel connect transaction planning with stable operation keys and rollback metadata, deterministic journal recovery after crash immediately after `004_route_full_tunnel`, and dry-run typed operation execution with journal writes, status transitions, first-error stop and applied-prefix rollback | real `utun`, `route`, `scutil`, `pfctl`, DNS/firewall/system proxy mutation, helper runtime, actual rollback execution, idempotent repeated commands, packet flow |
