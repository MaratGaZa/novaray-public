...
| Network transaction contract | pure `NetworkSnapshot` / `AppliedNetworkState` validation covers route/DNS/firewall snapshots, typed operation descriptors, transaction phases, duplicate operation/route rejection, bounded identifiers, missing rollback metadata rejection, explicit `apply_order`, reverse-order rollback step generation, failed/rolled-back semantics, unknown-field rejection, redacted `Debug` output, and typed recovery journal persistence with private directory, temp-write/fsync/rename, fail-closed per-file quarantine for corrupt/invalid state, orphan temp cleanup, file-safe transaction IDs, explicit clear, and redacted journal diagnostics | real `utun`, `route`, `scutil`, `pfctl`, DNS/firewall/system proxy mutation, helper runtime, actual rollback execution, packet flow |
