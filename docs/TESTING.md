...
| macOS helper runtime authentication boundary | ADR-009 proposes ordered admission through kernel-derived Unix peer credentials, a bounded connection-local Authorization Services external form, helper-side named-right checks before session and mutation, server-generated session identity and fail-closed redaction/lifetime rules | live Unix socket, kernel credential adapter, dedicated authorization right lifecycle, same-UID negative system tests, helper-side Security framework calls, root runtime and any network mutation |
