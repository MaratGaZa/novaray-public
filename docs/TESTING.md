...
| Generator | basic inbounds/outbounds, standard TLS (`allowInsecure: false`), Reality settings with canonical Raw URL-safe Base64 `password` normalization and defaults, normalized WebSocket `wsSettings`, gRPC `grpcSettings`, opt-in real Xray `v26.3.27` pre-flight for TLS WS/gRPC and Reality gRPC, and controlled loopback HTTP-over-SOCKS5 traffic for WS/gRPC | remote TLS/CDN/Reality interoperability, routing, DNS |
