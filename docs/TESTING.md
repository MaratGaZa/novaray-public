...
- macOS helper safe source opener contract: file-backed preflight opens a regular helper source file
  with a no-follow final symlink policy on Unix/macOS, hashes through the opened handle, rewinds the
  handle for future copy work, and rejects symlink/non-regular sources without retaining verified
  source state;
