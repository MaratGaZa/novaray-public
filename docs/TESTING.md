...
- macOS helper runtime replay guard contract: pure unit tests validate current-session command
  ...
  zero/repeated/stale sequence rejection, sequence reset only through a new handshake session,
  bounded session/correlation validation, correlation ID not acting as freshness proof, and
  session/correlation redaction in `Debug`;
