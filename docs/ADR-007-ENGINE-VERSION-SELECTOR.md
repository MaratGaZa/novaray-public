...
ADR-005 introduced the versioned engine catalog. ADR-006 bound configuration dialects to exact
...
- Let `--expected-sha256` imply an uncatalogued version: rejected because it would bypass ADR-006's
...
evidence in ADR-006. Revisit this ADR before adding uncatalogued/unsafe binary selection or runtime
download/update.
