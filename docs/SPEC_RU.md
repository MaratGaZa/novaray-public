...
- [x] macOS helper install/deinstall executor: core выполняет validated Gate I plans через typed
  ...
  verified opened source handle, применяет install order copy → plist → load, применяет uninstall
  order unload → remove plist → remove helper, возвращает typed execution/rollback/stop-state
  diagnostics. Concrete file-system adapter writes files and invokes `/bin/launchctl` через argv
...
- macOS helper install/deinstall executor обязан использовать только typed platform adapter:
  ...
  принимает shell string и не доказывает helper runtime или network behavior.
