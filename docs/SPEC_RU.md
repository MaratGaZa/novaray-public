...
Каждый критичный ключ (`flow`, `security`, `type`, `headerType`, `sni`, `pbk`, `sid`, `fp`,
`encryption`, `host`, `path`, `serviceName`, `authority`, `mode`) допускается не более одного раза.
Повторы проверяются по декодированному имени до интерпретации query-значений; одинаковые и пустые
значения не являются исключением. Сообщение повтора (`Display`) содержит только категорию и
каноническое имя, без URI, значений или адреса сервера. Стандартный `Debug` для `anyhow::Error`
может добавлять backtrace и не является форматом диагностического экспорта.
Разные alias сохраняют прежнюю проверку конфликтов;
неизвестные ключи по-прежнему игнорируются. Это не полная redaction всех ошибок импортера,
не ограничение размера URI и не доказательство сетевой безопасности.
...
Этот вариант требует документированного решения M7 об отсрочке: per-app capability и UI не
объявляются готовыми. Per-app evidence в ADR-003 Gate H и ADR-004 обязательно для релиза с per-app,
а не для domain/IP-only релиза с такой отсрочкой. Остальные network, DNS, recovery и release gates
остаются обязательными; отсрочка не переводит ADR в `Accepted` автоматически.
...
- Подготовительный `EndpointRevocation` должен владеть старой allowlist и binding, проверять
  active deny и известное состояние, записывать intent до мутаций и повторять наблюдение после
  записи. Порядок: остановка транспорта → проверка → удаление старого endpoint-исключения
  → проверка → очистка established-state → проверка отсутствия всех трёх → запись результата.
  Каждая проверка повторяет binding/deny; уже отозванные части не должны появляться снова.
  Любой отказ терминален, с сохранением причины и признаков возможного журнала/попытки мутации;
  нет retry, слепой компенсации, восстановления старого tuple или выдачи нового разрешения.
  Это только recording-adapter часть E04: `&mut` не доказывает системный lifecycle lock,
  наблюдения не удостоверяют ОС, а результат не является capability. Native adapter, durable
  recovery и packet-level evidence остаются открытыми, Gate H этим срезом не закрывается.
- При любом `EndpointRevocationError` с `mutation_attempted = true` будущий вызывающий executor
  обязан инициировать полный teardown затронутого VPN-соединения, а не только отменить замену:
  остановить engine/transport, освободить ресурсы соединения и отозвать все принадлежащие ему
  endpoint-исключения и established flows с проверкой результата, сохраняя deny. Это отдельный
  ограниченный по времени containment/recovery-путь, не retry отзыва и не обычный disconnect
  с восстановлением незащищённого egress. До подтверждения teardown запрещены новые grant,
  автоматический reconnect и direct DNS; незавершённый intent сохраняется. При сбое teardown
  сохраняются первичная ошибка и отдельная ошибка cleanup, состояние остаётся recovery/unknown,
  без заявления об отсутствии утечки. `mutation_attempted = false` также не доказывает безопасную
  сеть. `Err`/`Blocked` означает остановку последовательности, не системное fail-closed состояние.
- Подготовительная core-композиция должна потреблять объект отзыва и делать сырой путь без
  containment непубличным. После возвращённой ошибки с попыткой мутации она вызывает teardown
  ровно один раз, затем (только при успехе callback) независимо проверяет снимок: владельца
  ...
  durable recovery и packet-level evidence остаются открытыми; Gate H не закрыт.
- Отказ до первой мутации должен возвращать `RecoveryRequiredBeforeMutation(PreMutationRecovery)`:
  `DenyUnproven` при неподтверждённом deny, `StateUnverified` при остальных причинах. Первичные
  stage/cause и `journal_may_exist` сохраняются. `Inactive` и `Unknown` оба не подтверждают защиту;
  `DenyUnproven` не утверждает, что firewall достоверно выключен. Ноль собственных мутаций не
  означает безопасную сеть или отсутствие работы: вызывающий слой обязан передать отказ в
  recovery-оценку, запретить обычное продолжение, новый grant, reconnect и direct DNS, сохранить
  возможный intent до отдельного подтверждённого восстановления. Core не вызывает teardown
  до мутаций: отсутствие проверенного владения/контекста не разрешает слепую очистку. Это
  типизированное требование к будущему вызывающему слою, не выполненный recovery или native
  deny repair. Display оболочки ошибки описывает исход containment, а первичная причина доступна
  через `primary`/`source` и не дублируется при печати цепочки.
- Начальный pure-core `EndpointBootstrap` должен проверять явно подтверждённую незащищённую
  bootstrap-фазу без recovery/always-on и связывать ответ с поколениями сессии, запроса, профиля
  и сети. Лимиты: 64 записи, 16 уникальных IP, не более 30 секунд от начала до выдачи endpoint.
  Повторы используют минимальный deadline; непригодные адреса/семьи и истёкшие кандидаты не
  выбираются. Порядок — IPv4 перед IPv6, затем адрес; порт ненулевой и transport заданы профилем.
  Перед однократной выдачей повторно проверяются допуск, текущий контекст и монотонное время.
  Старый ответ отвергается без выбора; изменившийся текущий контекст, регресс времени или срок
  переводят модель в терминальный Blocked. Отправленные наблюдения не доказывают истину ОС,
  а результат — данные, не разрешение на привилегированное действие. Это только начальный
  выбор: без DNS-вызовов, active session, IP-literal flow, retry/ротации E04 и подключения к executor.
- Предлагаемый lifecycle DNS-bootstrap и смены endpoint задаёт
  [единый протокол](./ENDPOINT_BOOTSTRAP_PROTOCOL.md), пока без runtime-реализации: начальное
  разрешение имени вне VPN допустимо лишь в явно незащищённой bootstrap-фазе без active/inherited
  deny или recovery. При активной/неопределённой защите direct DNS запрещён, даже при reconnect.
  Один tuple закрепляется за сессией и проверенным network context; A/AAAA-кандидаты переключаются
  последовательно, с отзывом старого разрешения до нового. Истёкший снимок не разрешает новую
  попытку; смена сети инвалидирует его и ведёт в Blocked, а не к автоматическому DNS-исключению.
  Выбранный IP не меняет TLS/SNI/Reality identity профиля. Новый незащищённый bootstrap требует
  явного прекращения защиты пользователем и проверенного rollback. Это требования будущего
  Gate H, не доказательство работающего resolver, reconnect, always-on или отсутствия утечек.
...
- Перед новым исполнением плана с включённым kill switch core требует единственную операцию
  ...
  Компенсация маршрутов/DNS предшествует восстановлению firewall. Старые журналы читаются и
  восстанавливаются по сохранённому порядку, без применения нового admission-правила к recovery.
  Это контракт порядка, а не доказательство системного deny state или отсутствия утечек.
...
- Диагностика ошибки отзыва endpoint должна сохранять первичные stage/cause и вторичный исход,
  ...
  Запись хранит приватную проекцию кодов и флагов, а не доменные ошибки. Production-преобразование
  использует исчерпывающие match без wildcard для всех исходных enum: новый вариант требует
  явного выбора безопасного кода. Доменные ошибки не имеют Serialize; их будущий payload
  не попадает автоматически ни в JSON, ни в Debug диагностической записи.
...
- `RevocationDiagnosticBuffer` хранит в памяти только проекции ошибок отзыва из задачи 75.
  Вместимость 1–64 проверяется до выделения хранилища. Записи идут от старой к новой;
  при заполнении новая запись вытесняет ровно одну старейшую, повторы не объединяются.
  `dropped_records: u64` точно считает вытеснения с последней очистки; переполнение отклоняет
  добавление до изменения записей или счётчика. Ошибка буфера не заменяет первичную ошибку сети.
  Заимствованный неизменяемый Serialize-only снимок имеет только `schema_version = 1`, `capacity`,
  `dropped_records`, `records`; формат каждой записи остаётся v1. Компактный serde_json снимка
  ограничен 33 KiB (64 записи по 512 байт плюс разделители и оболочка); pretty/custom форматы
...
- `RevocationDiagnosticSnapshot::encode_json()` подготавливает собственный неизменяемый
  ...
  превышение даёт `SizeLimitExceeded`, отказ резервирования payload — `AllocationFailed`,
  другой отказ сериализации — `SerializationFailed`.
  ...
  а не к защите от любого OOM. Writer начинает без allocation; перед копированием проверяет
  размер и при необходимости выполняет fallible reserve с геометрическим ростом запрошенной
  вместимости не выше 33 KiB. Внутренний лимит также ограничивается этой константой до арифметики
  и reserve; публичного настраиваемого лимита нет. Первый отказ размера/reserve защёлкивается,
  повторной попытки и частичного результата нет. Ошибка не содержит сообщения аллокатора.
  Отказ reserve проверяется детерминированной инъекцией, не исчерпанием памяти хоста; abort
  аллокатора, прочие allocations serializer и общий RSS этим контрактом не ограничиваются.
  Panic/OOM/crash не перехватываются. Произвольные serializers/payloads,
