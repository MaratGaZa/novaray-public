...
- **Gate H (Helper Runtime — требуется для утверждения этого ADR):**
  ...
  - Перед deny state должны быть проверены разрешения точного VPN-endpoint через подтверждённый
    uplink и трафика через подтверждённый туннель. На реальных пакетах требуется показать
    сохранение endpoint/tunnel reachability, блокировку direct DNS/постороннего egress и
    неподдерживаемого IPv6, а при частичном отказе — локальный rollback без доступной сети.
    Отдельное evidence требуется для DHCP/NDP/bootstrap и смены интерфейса при reconnect;
    pure matcher и строковое имя интерфейса не закрывают эти критерии.
  - Для релиза с per-app правило направляет трафик выбранного приложения в отдельный outbound на
    реальном трафике. Domain/IP-only релиз допустим только с документированной отсрочкой M7 по
    FR-006; недоказанная per-app capability не объявляется доступной.
...
Уточнение 2026-09-18 (задача 70): [протокол endpoint bootstrap](./ENDPOINT_BOOTSTRAP_PROTOCOL.md)
предлагает resolve-before-protection только для явно незащищённого первого подключения;
при active/inherited/unknown deny прямой DNS запрещён. Смена сети или исчерпание пригодных
кандидатов ведут в Blocked, не к временному отключению kill switch. Документ фиксирует
альтернативы, цену ограничения reconnect, отзыв tuple и условия пересмотра; runtime не реализован.
Gate H дополнительно требует все сценарии [матрицы bootstrap/reconnect](./TESTING.md#endpoint-bootstrap-и-reconnect-задача-70),
включая несколько A/AAAA, ротацию адреса, поздние ответы, истечение срока, смену сети и crash.
Прежний критерий восстановления исходной конфигурации не означает автоматическое снятие активной
защиты при неожиданном сбое: различие protected recovery и явного disconnect должно быть доказано.
Статус ADR остаётся `Proposed`; этот документ не принимает production resolver/firewall решение.

Уточнение 2026-09-17 (задача 69): `KillSwitchAllowlist` задаёт только проверяемый Rust-контракт
для exact endpoint и одной семьи трафика туннеля. Альтернатива «разрешить весь uplink либо весь
трафик к endpoint независимо от порта» исключена как слишком широкая. Неявные DHCP/NDP/DNS
исключения отложены до отдельного контракта; без них модель не годится как полный системный ruleset.
Привязка policy к `ApplyFirewallPolicy`, верификация интерфейсов и компиляция правил ещё отсутствуют.
Backend, signing, entitlements и привилегии этой задачей не меняются; новых внешних фактов об Apple
API нет. ADR остаётся `Proposed`. Контракт пересматривается при разработке native adapter и по
результатам Gate H; откат pure-core среза не требует миграции журналов или изменения системного состояния.

Уточнение gates от 2026-09-13 согласует per-app условие с FR-006 и M7: альтернативой положительному
per-app evidence является явная отсрочка этой функции, а не её имитация. Требовать per-app для любого
релиза означало бы отменить domain/IP fallback SPEC. Остальные критерии Gate I/H, включая DNS и
recovery, не ослаблены; статус ADR остаётся `Proposed`. Перед включением per-app условие открывается
повторно и требует platform-specific packet evidence. Новые факты об Apple API этой ревизией не
заявляются.
