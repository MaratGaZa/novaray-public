...
4. **Архитектурная граница (FFI / ABI):**
   ...
   - Контракт остаётся UI-нейтральным: любой второй стек обязан использовать тот же набор команд и событий без собственной policy-логики (ADR-006, пункт 2). Именно это делает пункт 3 отложенным без риска.
5. **Сетевая граница:**
