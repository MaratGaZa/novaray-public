...
После слияния PR #129 задача 79 находится в `main`. Задача 80 / issue #130 / PR #131:
**Отказоустойчивое выделение памяти JSON-preview** реализована и проверена локально.
Это только поведение in-memory encoder, без файлового sink, UI или системных операций.
...
72. [x] Барьер отзыва старого endpoint перед заменой — issue #114, PR #115:
...
73. [x] Обязательный containment после частичного отзыва — issue #116, PR #117:
    ...
    Локальное evidence: 6 новых unit tests и 1 integration; default 371 passed / 0 failed /
    5 ignored, feature 387 / 0 / 5; 1 compile-fail doctest, fmt/strict Clippy обоих режимов.
    Три временные мутации пойманы и восстановлены; metadata/self-test 13/57, ссылки 54/0,
    traceability 16/16/16. Doc-tests добавлены в CI; CI/review текущего head проверяются в PR.
...
74. [x] Явное recovery при отказе до мутаций — issue #118, PR #119:
    ...
    Локальное evidence: 3 новых unit tests и 1 integration; default 375 passed / 0 failed /
    5 ignored, feature 391 / 0 / 5; 1 compile-fail doctest, fmt/strict Clippy обоих режимов.
    Три временные мутации пойманы и восстановлены; metadata/self-test 14/58, links 54/0,
...
78. [x] Запись результата отзыва в буфер диагностики — issue #126, PR #127:
    ...
    Критерии: consume-once, порядок containment до записи, исходная ошибка/source не меняются;
    overflow не мешает containment, не очищает буфер, не вызывает retry или ложный успех.
...
79. [x] Ограниченный JSON-preview диагностики в памяти — issue #128, PR #129:
    ...
    типизированный отказ без partial output/сырого сообщения, буфер не меняется; preview
    переживает clear/drop буфера, не принимается из внешних данных и не даёт mutable access.
...
80. [x] Отказоустойчивое выделение памяти JSON-preview — issue #130, PR #131:
    ...
    Evidence: пять новых unit tests и пять пойманных/восстановленных мутаций; прежние v1 и
    external-consumer проверки сохранены. Default: 405 passed / 0 failed / 5 ignored;
    feature: 421 / 0 / 5; 10 doctest, fmt и strict Clippy обоих режимов прошли.
    Это L1-контроль reserve и существующий L3 consumer, не host OOM или system evidence.
