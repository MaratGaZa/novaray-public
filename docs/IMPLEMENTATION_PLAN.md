...
30. [x] Dry-run network operation executor contract — issue #30: добавить pure Rust core runner и
    ...
    в `apply_order`, пишет recovery journal до/после dry-run операции, обновляет operation statuses,
    останавливается на первой typed ошибке и доказывает rollback applied-prefix behavior без shell,
    helper runtime, root, `utun`, route/DNS/firewall mutation or packet-flow evidence.
