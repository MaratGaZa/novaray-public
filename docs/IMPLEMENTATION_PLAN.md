...
| 76 | `[ ]` | Ограниченный буфер диагностики ошибок отзыва | Накапливает только безопасные записи в памяти с FIFO, точным учётом вытеснений и неизменяемым снимком; не пишет файлы и не выполняет recovery. | [#122](https://github.com/MaratGaZa/novaray-public/issues/122) | TBD |

После слияния PR #121 задача 75 находится в `main`. Задача 76 / issue #122:
**Ограниченный буфер диагностики ошибок отзыва** — только core-хранилище в памяти.
Критерии проверяются отдельно; persistent logging backend, bundle export и recovery не доказаны.
...
76. [ ] Ограниченный буфер диагностики ошибок отзыва — issue #122, PR TBD:
