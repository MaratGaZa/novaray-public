...
- [ ] Обрабатывать IPv6 literals, IDN/punycode и duplicate query parameters.
  - [x] Задача 81 / issue #132 / PR #133: отклонять повторы декодированных критичных ключей
...
- [ ] Добавить bounded rotating logs и user-visible diagnostic events.
  ...
  - [x] Задача 78 / issue #126 / PR #127: opt-in запись возвращённого отказа после обязательного containment;
    ошибка буфера отдельно от исходного отказа, без автоматической инструментации UI/CLI/runtime.
  - [x] Задача 79 / issue #128 / PR #129: собственный compact JSON-preview с проверкой лимита
    во время кодирования; без файлового export, UI и разрешения на передачу данных.
  - [x] Задача 80 / issue #130 / PR #131: постепенное fallible-резервирование памяти preview,
    типизированный отказ без частичного JSON; не общая защита процесса от OOM.
...
- [x] Обновить ближайшую очередь после слияния базового native harness: пункт 63 включён в `main`,
...
- [ ] Сохранять защиту при engine crash/reconnect.
  ...
  - [x] Задача 74 / issue #118 / PR #119: явное recovery при отказе до мутаций; отдельный DenyUnproven,
